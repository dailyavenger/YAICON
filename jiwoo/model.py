...
class GBDataset(Dataset):
    def __init__(self):
        with open("C:\\Users\\Jiwoo\\Desktop\\node_embeddings_f(명철님 결과).json", 'r') as f:
            self.graph_data = json.load(f)
        with open('kg2text.json','r') as f:
            self.caption_data=json.load(f)

    def __getitem__(self, index):
        graph_embedding=self.graph_data[index]['embeddings'] #2차원배열 [20,1024]
        caption=self.caption_data[index]
        return {"graph_embedding":graph_embedding, "cation":caption}

    def __len__(self):
        return len(self.graph_data)

    def collate_fn(self, samples):
        collate_graph_embedding=torch.stack([sample['graph_embedding'] for sample in samples])
        collate_caption=[]
        for value in samples.values():
            collate_caption.append(value)
        return {'graph_embedding':collate_graph_embedding, 'caption':collate_caption}

class GraphBlipTraining_Stage1(L.LightningModule):
    def __init__(self, model):
        super().__init__()
        self.model=model

    def training_step(self,batch):
        x=batch['graph_embedding']
        y=batch['caption']
        loss=self.model(x,y,mode=1)
        return loss
...
model=GraphBLIP()
train_model=GraphBlipTraining_Stage1(model)
dataset=GBDataset()
train_loader = utils.data.DataLoader(dataset)
trainer = L.Trainer(devices=1, accelerator="gpu",limit_train_batches=10, max_epochs=10)
trainer.fit(model=train_model, train_dataloaders=train_loader)
