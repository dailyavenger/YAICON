from transformers import Blip2QFormerModel, Blip2QFormerConfig, MistralForCausalLM,BertTokenizer,AutoTokenizer
import torch
...
blip2qformer_config = Blip2QFormerConfig()
...
class GraphBLIP():
    def __init__(self):
        ...
        self.qformer=Blip2QFormerModel(config=blip2qformer_config)
...
#                list(self.language_projection.parameters())
    ...
    def load_state_dict(self, state_dict):
        for name, param in state_dict.items():
            if isinstance(self.__dict__[name], nn.Parameter):
                self.__dict__[name].data.copy_(param)
            elif isinstance(self.__dict__[name], nn.Module):
                self.__dict__[name].load_state_dict(param)
    ...
    def forward(self,graph_embeddings,caption,mode): #graph_embeddings:[batch_size,20,1024]
        if mode==1:
            ...
            embeddings = self.word_embeddings(text.input_ids).to(self.device)
            query_tokens = self.query_tokens.expand(graph_embeddings.shape[0], -1, -1).to(self.device)
            ...
            targets = torch.linspace(0,  bs - 1, bs, dtype=int).to(self.device)
    ...
    def __call__(self, graph_embeddings, caption, mode):
        return self.forward(graph_embeddings, caption, mode)
