...
from data_utils import concat_datasets, reorg_datasets_by_split
...
class RunnerBase:
    ...
    def __init__(self, cfg, task, model, job_id):
        self.config = cfg
        self.job_id = job_id
    ...
    def process_data(self, data_file):
        ...
        for item in data:
            graph_ids.append(item['graph_id'])
            embeddings.append(item['embeddings'])
            subgraph_texts.append(item['subgraph_text'])

        self.embeddings_tensor = torch.tensor(embeddings, dtype=torch.float32)
        self.graph_ids = graph_ids
        self.subgraph_texts = subgraph_texts
    ...
    def train_epoch(self, epoch):
        # train
        self.model.train()
        embeddings = self.embeddings_tensor
        subgraph_texts = self.subgraph_texts
        num_batches = len(embeddings) // self.config.run_cfg.batch_size
