...
from blip.blip2 import Blip2Base, disabled_train
...
class Blip2OPT(Blip2Base):
    ...
    def forward(self, samples):
        ...
        with self.maybe_autocast():
            graph_embeds = torch.tensor(gat, dtype=torch.float32).to(self.device)
        gat_atts = torch.ones(graph_embeds.size()[:-1], dtype=torch.long).to(self.device)

        query_tokens = self.Qformer.embeddings.word_embeddings.weight[
                       :graph_embeds.size(1), :
                       ].expand(graph_embeds.shape[0], -1, -1)
